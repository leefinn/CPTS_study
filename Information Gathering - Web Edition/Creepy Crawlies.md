...
```shell-session
xF1NN@htb[/htb]$ pip3 install scrapy
```
...
```shell-session
xF1NN@htb[/htb]$ wget -O ReconSpider.zip https://academy.hackthebox.com/storage/modules/144/ReconSpider.v1.2.zip
xF1NN@htb[/htb]$ unzip ReconSpider.zip 
```
...
```shell-session
xF1NN@htb[/htb]$ python3 ReconSpider.py http://inlanefreight.com
```
