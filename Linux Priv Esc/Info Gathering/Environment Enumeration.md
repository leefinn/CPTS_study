...
```shell-session
xF1NN@htb[/htb]$ cat /etc/os-release
...
```shell-session
xF1NN@htb[/htb]$ cat /etc/passwd | cut -f1 -d:
...
```shell-session
xF1NN@htb[/htb]$ grep "sh$" /etc/passwd
...
```shell-session
xF1NN@htb[/htb]$ getent group sudo
...
```shell-session
xF1NN@htb[/htb]$ cat /etc/fstab | grep -v "#" | column -t
...
```shell-session
xF1NN@htb[/htb]$ find / -type f -name ".*" -exec ls -l {} \; 2>/dev/null | grep htb-student
...
```shell-session
xF1NN@htb[/htb]$ find / -type d -name ".*" -ls 2>/dev/null
...
```shell-session
xF1NN@htb[/htb]$ ls -l /tmp /var/tmp /dev/shm
