...
```shell-session
xF1NN@htb[/htb]$ sudo nmap --open -oA inlanefreight_ept_tcp_1k -iL scope 
...
```shell-session
xF1NN@htb[/htb]$ sudo nmap --open -p- -A -oA inlanefreight_ept_tcp_all_svc -iL scope
...
```shell-session
xF1NN@htb[/htb]$ egrep -v "^#|Status: Up" inlanefreight_ept_tcp_all_svc.gnmap | cut -d ' ' -f4- | tr ',' '\n' | \                                                               
...
```shell-session
xF1NN@htb[/htb]$ dig axfr inlanefreight.local @10.129.203.101
...
```shell-session
xF1NN@htb[/htb]$ curl -s -I http://10.129.203.101 -H "HOST: defnotvalid.inlanefreight.local" | grep "Content-Length:"
...
```shell-session
xF1NN@htb[/htb]$ ffuf -w namelist.txt:FUZZ -u http://10.129.203.101/ -H 'Host:FUZZ.inlanefreight.local' -fs 15157
...
```shell-session
xF1NN@htb[/htb]$ sudo tee -a /etc/hosts > /dev/null <<EOT
...
$ cat inlanefreight_tcp_all_svc.nmap 
# Nmap 7.94SVN scan initiated Tue Jan 27 09:52:06 2026 as: nmap --open -p- -A -oA inlanefreight_tcp_all_svc 10.129.12.2
Nmap scan report for 10.129.12.2
Host is up (0.0091s latency).
Not shown: 62754 closed tcp ports (reset), 2770 filtered tcp ports (no-response)
...
|_http-server-header: Apache/2.4.41 (Ubuntu)
110/tcp  open  pop3     Dovecot pop3d
|_pop3-capabilities: RESP-CODES SASL UIDL PIPELINING STLS CAPA TOP AUTH-RESP-CODE
|_ssl-date: TLS randomness does not represent time
| ssl-cert: Subject: commonName=ubuntu
| Subject Alternative Name: DNS:ubuntu
| Not valid before: 2022-05-30T17:15:40
...
| Not valid before: 2022-05-30T17:15:40
|_Not valid after:  2032-05-27T17:15:40
|_imap-capabilities: more post-login have OK capabilities ID listed ENABLE IMAP4rev1 SASL-IR LOGINDISABLEDA0001 IDLE STARTTLS LITERAL+ Pre-login LOGIN-REFERRALS
993/tcp  open  ssl/imap Dovecot imapd (Ubuntu)
| ssl-cert: Subject: commonName=ubuntu
| Subject Alternative Name: DNS:ubuntu
| Not valid before: 2022-05-30T17:15:40
|_Not valid after:  2032-05-27T17:15:40
|_ssl-date: TLS randomness does not represent time
|_imap-capabilities: more AUTH=PLAINA0001 have post-login capabilities listed ENABLE IMAP4rev1 SASL-IR OK IDLE ID LITERAL+ Pre-login LOGIN-REFERRALS
995/tcp  open  ssl/pop3 Dovecot pop3d
|_pop3-capabilities: RESP-CODES SASL(PLAIN) UIDL PIPELINING USER CAPA TOP AUTH-RESP-CODE
|_ssl-date: TLS randomness does not represent time
| ssl-cert: Subject: commonName=ubuntu
| Subject Alternative Name: DNS:ubuntu
| Not valid before: 2022-05-30T17:15:40
...
|_http-title: Support Center
1 service unrecognized despite returning data. If you know the service/version, please submit the following fingerprint at https://nmap.org/cgi-bin/submit.cgi?new-service :
SF-Port53-TCP:V=7.94SVN%I=7%D=1/27%Time=6978DF3F%P=x86_64-pc-linux-gnu%r(D
...
No exact OS matches for host (If you know what OS is running on it, see https://nmap.org/submit/ ).
TCP/IP fingerprint:
OS:SCAN(V=7.94SVN%E=4%D=1/27%OT=21%CT=1%CU=41583%PV=Y%DS=2%DC=T%G=Y%TM=6978
OS:DF74%P=x86_64-pc-linux-gnu)SEQ(SP=103%GCD=1%ISR=10D%TI=Z%CI=Z%II=I%TS=8)
OS:SEQ(SP=104%GCD=1%ISR=10C%TI=Z%CI=Z%II=I%TS=A)OPS(O1=M552ST11NW7%O2=M552S
...
HOP RTT     ADDRESS
1   8.74 ms 10.10.14.1
2   9.06 ms 10.129.12.2

OS and Service detection performed. Please report any incorrect results at https://nmap.org/submit/ .
# Nmap done at Tue Jan 27 09:53:24 2026 -- 1 IP address (1 host up) scanned in 78.15 seconds
...
$ dig axfr inlanefreight.local @10.129.12.2

; <<>> DiG 9.18.33-1~deb12u2-Debian <<>> axfr inlanefreight.local @10.129.12.2
;; global options: +cmd
inlanefreight.local.	86400	IN	SOA	ns1.inlanfreight.local. dnsadmin.inlanefreight.local. 21 604800 86400 2419200 86400
inlanefreight.local.	86400	IN	NS	inlanefreight.local.
inlanefreight.local.	86400	IN	A	127.0.0.1
...
inlanefreight.local.	86400	IN	SOA	ns1.inlanfreight.local. dnsadmin.inlanefreight.local. 21 604800 86400 2419200 86400
;; Query time: 9 msec
;; SERVER: 10.129.12.2#53(10.129.12.2) (TCP)
;; WHEN: Tue Jan 27 09:57:38 CST 2026
;; XFR size: 14 records (messages 1, bytes 448)

- HTB{DNs_ZOn3_Tr@nsf3r}
---
$ curl -s -I http://10.129.12.2 -H "HOST: inlanefreight.local" | grep "Content-Length:"
Content-Length: 15157

$ ffuf -w subdomains-top1million-110000.txt:FUZZ -u http://10.129.12.2/ -H 'Host:FUZZ.inlanefreight.local' -fs 15157
