...
![Obsidian note-taking app showing a sidebar with sections like 'Administrative Information' and 'Scoping Information' under 'HTB Academy'.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/162/notetaking.png)
...
```shell-session
xF1NN@htb[/htb]$ git clone https://github.com/tmux-plugins/tpm ~/.tmux/plugins/tpm
```
...
```shell-session
xF1NN@htb[/htb]$ tmux source ~/.tmux.conf 
```
...
![GIF showcasing usage the tmux logging plugin.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/162/tmux_log_enable.gif)
...
![GIF showcasing a vertical split between two panes in tmux.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/162/tmux_pane_capture.gif)
...
```shell-session
xF1NN@htb[/htb]$ mkdir -p ACME-IPT/{Admin,Deliverables,Evidence/{Findings,Scans/{Vuln,Service,Web,'AD Enumeration'},Notes,OSINT,Wireless,'Logging output','Misc Files'},Retest}
```

  Notetaking & Organization

```shell-session
xF1NN@htb[/htb]$ tree ACME-IPT/
...
![Obsidian app with sidebar showing 'HTB_Academy' sections like 'Admin' and 'Evidence'; main area states 'No file is open' with file options.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/162/notetaking2.png)
...
![Obsidian app with sidebar showing 'Notes' sections like 'Administrative Information' and 'Scoping Information'; main area states 'No file is open' with file options.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/162/notetaking3.png)
...
![Terminal output showing CrackMapExec command against IP 172.16.5.5, indicating successful SMB connection to DC01 in INLANEFREIGHT.LOCAL domain.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/162/blurred.png)

#### Blanking Out Password with Solid Shape

![Terminal output showing CrackMapExec command against IP 172.16.5.5, indicating successful SMB connection to DC01 in INLANEFREIGHT.LOCAL domain.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/162/boxes.png)
...
![Terminal output showing CrackMapExec command against IP 172.16.5.5, indicating successful SMB connection to DC01 in INLANEFREIGHT.LOCAL domain with password redacted.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/162/terminal_output.png)
...
```shell-session
xF1NN@htb[/htb]$ xfreerdp /v:10.129.203.82 /u:htb-student /p:HTB_@cademy_stdnt!
```
