...
```shell-session
xF1NN@htb[/htb]$ xfreerdp /v:<MS01 target IP> /u:htb-student /p:Academy_student_AD!
```
...
```shell-session
xF1NN@htb[/htb]$ ssh htb-student@<ATTACK01 target IP>
```
...
```shell-session
xF1NN@htb[/htb]$ xfreerdp /v:<ATTACK01 target IP> /u:htb-student /p:HTB_@cademy_stdnt!
```
