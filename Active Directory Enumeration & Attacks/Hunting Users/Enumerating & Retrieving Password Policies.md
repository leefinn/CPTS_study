...
```shell-session
xF1NN@htb[/htb]$ crackmapexec smb 172.16.5.5 -u avazquez -p Password123 --pass-pol
...
```shell-session
xF1NN@htb[/htb]$ rpcclient -U "" -N 172.16.5.5
...
```shell-session
xF1NN@htb[/htb]$ enum4linux -P 172.16.5.5
...
```shell-session
xF1NN@htb[/htb]$ enum4linux-ng -P 172.16.5.5 -oA ilfreight
...
```shell-session
xF1NN@htb[/htb]$ cat ilfreight.json 
...
```shell-session
xF1NN@htb[/htb]$ ldapsearch -h 172.16.5.5 -x -b "DC=INLANEFREIGHT,DC=LOCAL" -s sub "*" | grep -m 1 -B 10 pwdHistoryLength
