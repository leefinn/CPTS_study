...
```shell-session
xF1NN@htb[/htb]$ python sqlmap.py -u 'http://inlanefreight.htb/page.php?id=5'
...
```shell-session
xF1NN@htb[/htb]$ sudo apt install sqlmap
```

If we want to install manually, we can use the following command in the Linux terminal or the Windows command line:

```shell-session
xF1NN@htb[/htb]$ git clone --depth 1 https://github.com/sqlmapproject/sqlmap.git sqlmap-dev
```

After that, SQLMap can be run with:

```shell-session
xF1NN@htb[/htb]$ python sqlmap.py
```
