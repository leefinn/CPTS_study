...
```shell-session
xF1NN@htb[/htb]$ sudo mkdir /mnt/Finance
xF1NN@htb[/htb]$ sudo mount -t cifs -o username=plaintext,password=Password123,domain=. //192.168.220.129/Finance /mnt/Finance
```
...
```shell-session
xF1NN@htb[/htb]$ mount -t cifs //192.168.220.129/Finance /mnt/Finance -o credentials=/path/credentialfile
```
...
```shell-session
xF1NN@htb[/htb]$ find /mnt/Finance/ -name *cred*
...
```shell-session
xF1NN@htb[/htb]$ grep -rn /mnt/Finance/ -ie cred
...
```shell-session
xF1NN@htb[/htb]$ sudo apt-get install evolution
...SNIP...
```
...
```shell-session
xF1NN@htb[/htb]$ sqsh -S 10.129.20.13 -U username -P Password123
```
...
```shell-session
xF1NN@htb[/htb]$ mysql -u username -pPassword123 -h 10.129.20.13
```
...
```shell-session
xF1NN@htb[/htb]$ sudo dpkg -i dbeaver-<version>.deb
```
