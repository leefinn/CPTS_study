...
```shell-session
xF1NN@htb[/htb]$ cat /etc/snmp/snmpd.conf | grep -v "#" | sed -r '/^\s*$/d'
...
```shell-session
xF1NN@htb[/htb]$ snmpwalk -v2c -c public 10.129.14.128
...
```shell-session
xF1NN@htb[/htb]$ sudo apt install onesixtyone
xF1NN@htb[/htb]$ onesixtyone -c /opt/useful/seclists/Discovery/SNMP/snmp.txt 10.129.14.128
...
```shell-session
xF1NN@htb[/htb]$ sudo apt install braa
xF1NN@htb[/htb]$ braa <community string>@<IP>:.1.3.6.*   # Syntax
xF1NN@htb[/htb]$ braa public@10.129.14.128:.1.3.6.*
