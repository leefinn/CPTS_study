...
![Process Monitor window with filter settings open. Filters include 'Operation is Load Image' and process names like main.exe included, others like Procmon.exe excluded.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/67/procmon.png)

Then if you scroll to the bottom, you can see the call to load `library.dll`.

![Process Monitor window displaying multiple entries for main.exe with operations like RegOpenKey, RegQueryValue, and Load Image. Results show SUCCESS and NAME NOT FOUND.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/67/procmon-loadimage.png)
...
![Command prompt showing a program that adds two numbers. Input: 1 and 1. Output: 'HIJACKED' message, adding 1 to the sum, resulting in 3 instead of 2.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/67/proxy.png)