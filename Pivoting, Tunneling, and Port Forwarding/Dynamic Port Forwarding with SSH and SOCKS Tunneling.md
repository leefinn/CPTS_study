...
![Diagram of SSH port forwarding: Attack Host (10.10.15.5) requests forwarding from local port 1234 to remote port 3306 on Victim Server (172.16.5.129, 10.129.15.50). SSH listens on local port 1234, forwards traffic to MySQL on localhost:3306, and returns via port 22.](https://academy.hackthebox.com/storage/modules/158/11.png)
...
```shell-session
xF1NN@htb[/htb]$ nmap -sT -p22,3306 10.129.202.64
...
```shell-session
xF1NN@htb[/htb]$ ssh -L 1234:localhost:3306 ubuntu@10.129.202.64
...
```shell-session
xF1NN@htb[/htb]$ netstat -antp | grep 1234
...
```shell-session
xF1NN@htb[/htb]$ nmap -v -sV -p1234 localhost
...
```shell-session
xF1NN@htb[/htb]$ ssh -L 1234:localhost:3306 -L 8080:localhost:80 ubuntu@10.129.202.64
```
...
![Diagram of Nmap scan setup: Attack Host (10.10.15.5) uses Proxychains and SSH Client to forward packets. SOCKS Listener on port 9050 forwards Nmap packets via SSH to Victim Server (10.129.15.50, 172.16.5.129) on port 22 for scanning.](https://academy.hackthebox.com/storage/modules/158/22.png)
...
```shell-session
xF1NN@htb[/htb]$ ssh -D 9050 ubuntu@10.129.202.64
```
...
```shell-session
xF1NN@htb[/htb]$ tail -4 /etc/proxychains.conf
...
```shell-session
xF1NN@htb[/htb]$ proxychains nmap -v -sn 172.16.5.1-200
...
```shell-session
xF1NN@htb[/htb]$ proxychains nmap -v -Pn -sT 172.16.5.19
...
```shell-session
xF1NN@htb[/htb]$ proxychains msfconsole
...
![Terminal running ProxyChains with xfreerdp command to connect to 172.16.5.19 as user 'victor'. Remote desktop shows Windows interface with Recycle Bin.](https://academy.hackthebox.com/storage/modules/158/proxychaining.png)
