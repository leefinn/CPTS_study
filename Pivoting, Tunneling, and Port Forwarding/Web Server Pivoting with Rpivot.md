...
![Network diagram: Attack Host (10.10.15.5) connects to External Network (10.129.110.0), then to Victim Server (Ubuntu) at 10.129.15.50, 172.16.5.129. Victim Server (Webserver) at 172.16.5.135 on Internal Network 172.16.5.0, running webserver on port 80.](https://academy.hackthebox.com/storage/modules/158/77.png)
...
```shell-session
xF1NN@htb[/htb]$ git clone https://github.com/klsecservices/rpivot.git
```
...
```shell-session
xF1NN@htb[/htb]$ sudo apt-get install python2.7
```
...
```shell-session
xF1NN@htb[/htb]$ curl https://pyenv.run | bash
xF1NN@htb[/htb]$ echo 'export PYENV_ROOT="$HOME/.pyenv"' >> ~/.bashrc
xF1NN@htb[/htb]$ echo 'command -v pyenv >/dev/null || export PATH="$PYENV_ROOT/bin:$PATH"' >> ~/.bashrc
xF1NN@htb[/htb]$ echo 'eval "$(pyenv init -)"' >> ~/.bashrc
xF1NN@htb[/htb]$ source ~/.bashrc
xF1NN@htb[/htb]$ pyenv install 2.7
xF1NN@htb[/htb]$ pyenv shell 2.7
```
...
```shell-session
xF1NN@htb[/htb]$ python2.7 server.py --proxy-port 9050 --server-port 9999 --server-ip 0.0.0.0
```
...
```shell-session
xF1NN@htb[/htb]$ scp -r rpivot ubuntu@<IpaddressOfTarget>:/home/ubuntu/
```
...
![Terminal running ProxyChains with Firefox connecting to 172.16.5.135. Browser displays Apache2 Ubuntu Default Page, confirming server setup.](https://academy.hackthebox.com/storage/modules/158/rpivot_proxychain.png)
...
```shell-session
python client.py --server-ip <IPaddressofTargetWebServer> --server-port 8080 --ntlm-proxy-ip <IPaddressofProxy> --ntl
```