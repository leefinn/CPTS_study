...
xF1NN@htb[/htb]$ sudo apt-get -y install medusa
```
...
```shell-session
xF1NN@htb[/htb]$ medusa [target_options] [credential_options] -M module [module_options]
```
...
```shell-session
xF1NN@htb[/htb]$ medusa -h 192.168.0.100 -U usernames.txt -P passwords.txt -M ssh 
```
...
```shell-session
xF1NN@htb[/htb]$ medusa -H web_servers.txt -U usernames.txt -P passwords.txt -M http -m GET 
```
...
```shell-session
xF1NN@htb[/htb]$ medusa -h 10.0.0.5 -U usernames.txt -e ns -M service_name
```
