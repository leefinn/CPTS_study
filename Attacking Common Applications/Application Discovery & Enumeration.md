...
```shell-session
xF1NN@htb[/htb]$ nmap -p 80,443,8000,8080,8180,8888,10000 --open -oA web_discovery -iL scope_list
```
...
```shell-session
xF1NN@htb[/htb]$ sudo  nmap -p 80,443,8000,8080,8180,8888,10000 --open -oA web_discovery -iL scope_list 
...
```shell-session
xF1NN@htb[/htb]$ sudo nmap --open -sV 10.129.201.50
...
```shell-session
xF1NN@htb[/htb]$ sudo apt install eyewitness
```
...
```shell-session
xF1NN@htb[/htb]$ eyewitness --web -x web_discovery.xml -d inlanefreight_eyewitness
...
```shell-session
xF1NN@htb[/htb]$ wget https://github.com/michenriksen/aquatone/releases/download/v1.7.0/aquatone_linux_amd64_1.7.0.zip
```

  Application Discovery & Enumeration

```shell-session
xF1NN@htb[/htb]$ unzip aquatone_linux_amd64_1.7.0.zip 
...
```shell-session
xF1NN@htb[/htb]$ cat web_discovery.xml | ./aquatone -nmap
...
![The image shows a report with a table of contents listing categories like High Value Targets, Uncategorized, Content Management System (CMS), 401/403 Unauthorized, and Splash Pages. It includes a section titled "High Value Targets" with web request info and a screenshot of an Apache Tomcat page. The report was generated on 09/07/2021.](https://academy.hackthebox.com/storage/modules/113/eyewitness4.png)
...
![Web report showing Inlanefreight homepage and helpdesk info, including server details and response codes.](https://academy.hackthebox.com/storage/modules/113/eyewitness3.png)
