...
```shell-session
xF1NN@htb[/htb]$ ldapsearch -H ldap://ldap.example.com:389 -D "cn=admin,dc=example,dc=com" -w secret123 -b "ou=people,dc=example,dc=com" "(mail=john.doe@example.com)"
...
```shell-session
xF1NN@htb[/htb]$ nmap -p- -sC -sV --open --min-rate=1000 10.129.204.229
