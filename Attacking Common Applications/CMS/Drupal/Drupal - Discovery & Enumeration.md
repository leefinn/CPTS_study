...
```shell-session
xF1NN@htb[/htb]$ curl -s http://drupal.inlanefreight.local | grep Drupal
...
![Inlanefreight Blog page with a 'Coming soon' message for a new employee blog and industry news site.](https://academy.hackthebox.com/storage/modules/113/drupal_node.png)
...
```shell-session
xF1NN@htb[/htb]$ curl -s http://drupal-acc.inlanefreight.local/CHANGELOG.txt | grep -m2 ""
...
```shell-session
xF1NN@htb[/htb]$ curl -s http://drupal.inlanefreight.local/CHANGELOG.txt
...
```shell-session
xF1NN@htb[/htb]$ droopescan scan drupal -u http://drupal.inlanefreight.local
