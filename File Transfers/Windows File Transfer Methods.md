...
```shell-session
xF1NN@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare
...
```shell-session
xF1NN@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare -user test -password test
...
```shell-session
xF1NN@htb[/htb]$ sudo pip3 install pyftpdlib
```
...
```shell-session
xF1NN@htb[/htb]$ sudo python3 -m pyftpdlib --port 21
...
```shell-session
xF1NN@htb[/htb]$ echo  | base64 -d > hosts
```

  Windows File Transfer Methods

```shell-session
xF1NN@htb[/htb]$ md5sum hosts 
...
```shell-session
xF1NN@htb[/htb]$ pip3 install uploadserver
...
```shell-session
xF1NN@htb[/htb]$ python3 -m uploadserver
...
```shell-session
xF1NN@htb[/htb]$ echo <base64> | base64 -d -w 0 > hosts
```
...
```shell-session
xF1NN@htb[/htb]$ sudo pip3 install wsgidav cheroot
...
```shell-session
xF1NN@htb[/htb]$ sudo wsgidav --host=0.0.0.0 --port=80 --root=/tmp --auth=anonymous 
...
```shell-session
xF1NN@htb[/htb]$ sudo python3 -m pyftpdlib --port 21 --write
