...
```shell-session
xF1NN@htb[/htb]$ sudo mkdir -p /var/www/uploads/SecretUploadDirectory
```
...
```shell-session
xF1NN@htb[/htb]$ sudo chown -R www-data:www-data /var/www/uploads/SecretUploadDirectory
```
...
xF1NN@htb[/htb]$ sudo ln -s /etc/nginx/sites-available/upload.conf /etc/nginx/sites-enabled/
```
...
```shell-session
xF1NN@htb[/htb]$ sudo systemctl restart nginx.service
```
...
```shell-session
xF1NN@htb[/htb]$ tail -2 /var/log/nginx/error.log
...
```shell-session
xF1NN@htb[/htb]$ sudo rm /etc/nginx/sites-enabled/default
```
...
```shell-session
xF1NN@htb[/htb]$ curl -T /etc/passwd http://localhost:9001/SecretUploadDirectory/users.txt
```

  Catching Files over HTTP/S

```shell-session
xF1NN@htb[/htb]$ sudo tail -1 /var/www/uploads/SecretUploadDirectory/users.txt 
