...
Now, we can do the same thing we did before by copying our payload, pasting it in our HTTP request in `Burp Suite`, URL-encoding it, and then finally sending it: ![Interface showing an HTTP request and response. The request includes headers like Host and User-Agent, with IP '127.0.0.1+%26%26+whoami'. The response displays HTML with a ping result for 127.0.0.1.](https://academy.hackthebox.com/storage/modules/109/cmdinj_basic_AND.jpg)
...
As we can see, this time, the `whoami` command did execute after the `ping` command failed and gave us an error message. So, let us now try the (`|| whoami`) payload in our HTTP request: ![Interface showing an HTTP request and response. The request includes headers like Host and User-Agent, with IP set to '|+whoami'. The response displays HTML with a form for entering an IP address and a 'Check' button.](https://academy.hackthebox.com/storage/modules/109/cmdinj_basic_OR.jpg)
