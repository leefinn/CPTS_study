...
```shell-session
xF1NN@htb[/htb]$ ssh Bob@10.10.10.10
...
```shell-session
xF1NN@htb[/htb]$ netcat 10.10.10.10 22
...
```shell-session
xF1NN@htb[/htb]$ sudo apt install tmux -y
```
...
If we want to save a file or quit `Vim`, we have to press`:` to go into `command mode`. Once we do, we will see any commands we type at the bottom of the vim window: ![Parrot Terminal displaying a configuration file with network settings and comments.](https://academy.hackthebox.com/storage/modules/77/getting_started_vim_3.jpg)
