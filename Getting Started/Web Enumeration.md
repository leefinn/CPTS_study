...
```shell-session
xF1NN@htb[/htb]$ gobuster dir -u http://10.10.10.121/ -w /usr/share/seclists/Discovery/Web-Content/common.txt
...
```shell-session
xF1NN@htb[/htb]$ git clone https://github.com/danielmiessler/SecLists
```

  Web Enumeration

```shell-session
xF1NN@htb[/htb]$ sudo apt install seclists -y
```
...
```shell-session
xF1NN@htb[/htb]$ gobuster dns -d inlanefreight.com -w /usr/share/SecLists/Discovery/DNS/namelist.txt
...
```shell-session
xF1NN@htb[/htb]$ curl -IL https://www.inlanefreight.com
...
```shell-session
xF1NN@htb[/htb]$ whatweb 10.10.10.121
...
```shell-session
xF1NN@htb[/htb]$ whatweb --no-errors 10.10.10.0/24
