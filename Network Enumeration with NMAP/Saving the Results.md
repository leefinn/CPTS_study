...
```shell-session
xF1NN@htb[/htb]$ sudo nmap 10.129.2.28 -p- -oA target
...
```shell-session
xF1NN@htb[/htb]$ cat target.gnmap
...
```shell-session
xF1NN@htb[/htb]$ xsltproc target.xml -o target.html
```
