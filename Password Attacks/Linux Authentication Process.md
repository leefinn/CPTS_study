...
```shell-session
xF1NN@htb[/htb]$ head -n 1 /etc/passwd
...
```shell-session
xF1NN@htb[/htb]$ sudo cat /etc/security/opasswd
...
```shell-session
xF1NN@htb[/htb]$ sudo cp /etc/passwd /tmp/passwd.bak 
xF1NN@htb[/htb]$ sudo cp /etc/shadow /tmp/shadow.bak 
xF1NN@htb[/htb]$ unshadow /tmp/passwd.bak /tmp/shadow.bak > /tmp/unshadowed.hashes
```
...
```shell-session
xF1NN@htb[/htb]$ hashcat -m 1800 -a 0 /tmp/unshadowed.hashes rockyou.txt -o /tmp/unshadowed.cracked
```
