...
```shell-session
xF1NN@htb[/htb]$ john --single passwd
...
```shell-session
xF1NN@htb[/htb]$ john --wordlist=<wordlist_file> <hash_file>
```
...
```shell-session
xF1NN@htb[/htb]$ john --incremental <hash_file>
```
...
```shell-session
xF1NN@htb[/htb]$ grep '# Incremental modes' -A 100 /etc/john/john.conf
...
```shell-session
xF1NN@htb[/htb]$ hashid -j 193069ceb0461e1d40d216e32c79c704
...
```shell-session
xF1NN@htb[/htb]$ <tool> <file_to_crack> > file.hash
```
